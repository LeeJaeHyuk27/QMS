...
public class PlanController {
	...
	// 생산 실적 모달
	@RequestMapping("/plan/selectModalInfo")
	@ResponseBody
	public ProductVO selectModalInfo(@ModelAttribute ProductVO vo)throws Exception{

		List<ProductVO> list = service.selectModalInfo(vo);
	    List<ProductVO> user = service.selectAllUser(vo);
	...
	// 생산 실적 등록
	@RequestMapping("/plan/deleteInsertitem")
	@ResponseBody
	public int deleteInsertitem(@ModelAttribute("ProductVO")ProductVO vo)throws Exception{
	...
	// 생산실적 다운로드
    @PostMapping("/plan/downloadProductExcel")
	public ResponseEntity<byte[]> downloadProductExcel(@ModelAttribute("ProductVO")ProductVO vo) throws Exception {
	    Map<String, Object> parameters = new HashMap<>();
	    ...
	    List<Map<String, Object>> dataList = excelService.selectProductQtyToExcel(vo);
	    for(int i = 1; i <=cal.getActualMaximum(Calendar.DAY_OF_MONTH); i++) {
	    	dateHeaders[i-1] = i+"일";
	    	dateCol[i-1] = "d"+i;
	    }
	    ...
	    String sheetName = "생산실적조회";
	    String filenName = "ProductQty.xlsx";
	    return excelService.createExcelFile(dataList, arrCol, arr3, filenName , sheetName);
	}
}
